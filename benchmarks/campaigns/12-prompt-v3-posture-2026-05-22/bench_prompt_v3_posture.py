...
- B_v3_posture: prompt_version="v3" (V3 nova com person_position_signature)
...
def run_gate_call(window: Window, prompt_version: str) -> dict:
    ...
    try:
        ...
        request_id = f"bench12-{window.window_id[:12]}-{prompt_version}-{uuid.uuid4().hex[:4]}"
        t0 = time.monotonic()
...
ARM_SPECS = {
    ...
    "B_v3_posture":  {"prompt_version": "v3", "description": "V3 (postura corporal + LOCAL_CONTEXT)"},
}
...
def main() -> int:
    ...
    p.add_argument("--arms", default="A_v2_baseline,B_v3_posture")
