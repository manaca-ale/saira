...
def run_gate_call(window: Window, prompt_label: str, prompt_text: str) -> dict:
    ...
    try:
        request_id = f"bench18-{window.window_id[:8]}-{prompt_label}-{uuid.uuid4().hex[:4]}"
        t0 = time.monotonic()
        ...
        return _serialize_gate_result(result, window, wall_ms, prompt_label)
...
ARM_SPECS = {
    "A_v3": {
        "description": "V3 original posture-first",
        "prompt_label": "v3",
        "prompt_text": _prompts_v3.NEW_LITTER_SYSTEM_PROMPT_V3,
    },
    "B_v3_esp32_recall": {
        "description": "V3 + recall mode esp32_002",
        "prompt_label": "v3_esp32_recall",
        "prompt_text": _prompts_v3.NEW_LITTER_SYSTEM_PROMPT_V3 + "\n\n" + ESP32_RECALL_BLOCK,
    },
    "C_v3_esp32_recall_b2": {
        "description": "V3 + recall mode esp32_002 B2 (material-transfer required)",
...
def main() -> int:
    ...
    parser.add_argument("--arms", default="A_v3,B_v3_esp32_recall")
