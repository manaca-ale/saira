#!/usr/bin/env python3
"""bench_prompt_v3_posture.py — Campanha 12 — A/B V2 (baseline) vs V3 (posture).

Testa se V3 (postura corporal como sinal primário) recupera os 4 TPs que V2
perdeu na camp 11 sem regredir as melhorias de V2 em rejeição de FPs.

2 arms (thinking_budget=1024 fixo nos dois):
- A_v2_baseline: prompt_version="v2" (V2 com fix carroça da camp 11)
- B_v3_2_collection_fix: prompt_version="v3" (V3.1 prompt mais conservador + 3-signal gate)

CAMERA_CONTEXT inclui `gemini_context_notes` com padrões conhecidos por câmera
(Imbiribeira e Mangabeira) — V3 e V2 builders injetam isso como LOCAL_CONTEXT
no prompt. Por enquanto o context é hardcoded aqui; em prod virá do DB.

Adaptado de bench_prompt_v2_promotion.py (camp 11). Diferenças:
- ARM_SPECS reflete V2 vs V3.
- CAMERA_CONTEXT inclui gemini_context_notes por câmera.
- Output dict inclui person_position_signature (campo V3-only).
"""
...
THINKING_BUDGET_BOTH = 1024


# Camera context with LOCAL_CONTEXT (gemini_context_notes) populated by hand for camp 12.
# In production this will come from the cameras.gemini_context_notes column (future migration).
IMBIRIBEIRA_NOTES = (
    "Câmera em poste alto sobre terreno baldio aberto. Pilha grande e permanente de "
    "entulho/lixo à esquerda do enquadramento. Descartes acontecem em sua maioria à "
    "noite/madrugada por pedestres com sacos pequenos (volumetria 0,01-0,15 m³, "
    "invisível na resolução), e ocasionalmente por carroceiros que param brevemente. "
    "Coleta da prefeitura passa de dia em caminhão compactador EMLURB. Chuva é "
    "frequente e gera borrões nas imagens noturnas."
)

MANGABEIRA_NOTES = (
    "Câmera em vista frontal/lateral sobre lixeira pública informal na esquina, com "
    "calçada à direita e via à esquerda. Descartes acontecem ao longo do dia por "
    "moradores a pé (1-2 sacos), pessoas com carrinho de mão, e ocasionalmente "
...
CAMERA_CONTEXT = {
    "cam_mangabeira": {
        ...
        "bairro": "Mangabeira",
        "rpa": "RPA 3",
        "gemini_context_notes": MANGABEIRA_NOTES,
    },
    "cam_imbiribeira": {
        ...
        "bairro": "Imbiribeira",
        "rpa": "RPA 2",
        "gemini_context_notes": IMBIRIBEIRA_NOTES,
    },
}
...
def run_gate_call(window: Window, prompt_version: str) -> dict:
    ...
    try:
        ...
        request_id = f"bench14-{window.window_id[:12]}-{prompt_version}-{uuid.uuid4().hex[:4]}"
        t0 = time.monotonic()
        ...
        posture = getattr(report, "person_position_signature", None)  # V3 only
        return {
            "ok": True,
            "prompt_version": prompt_version,
            ...
            "person_position_signature": posture,
...
ARM_SPECS = {
    "A_v2_baseline":     {"prompt_version": "v2", "description": "V2 com fix carroça (baseline camp 11)"},
    "B_v3_2_collection_fix":   {"prompt_version": "v3", "description": "V3.1 (posture stricter + 3-signal gate + neg examples)"},
}
...
def main() -> int:
    ...
    p.add_argument("--arms", default="A_v2_baseline,B_v3_2_collection_fix")
    ...
    print(f"Thinking budget (fixo nos 2 arms): {THINKING_BUDGET_BOTH}", flush=True)
    ...
    print(f"  {len(event_windows)} eventos; {n_with_mids} com 3 mid_frames", flush=True)
    ...
    if args.skip_baseline:
        ...
        print(f"  {len(baseline_windows)} baseline ({args.baseline_per_series or 'all'}/série)", flush=True)
    ...
    print(f"\n=== Resumo ===", flush=True)
    print(f"{'Arm':16s} {'N':>5s} {'OK':>5s} {'Cost USD':>10s} {'Tempo':>8s}", flush=True)
    for s in summary:
        print(f"{s['arm']:16s} {s['n']:5d} {s['n_ok']:5d} {s['cost_usd']:10.4f} "
