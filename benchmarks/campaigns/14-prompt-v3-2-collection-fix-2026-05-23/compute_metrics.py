...
Reads results-{A_v2_baseline,B_v3_2_collection_fix}.json from bench_prompt_v3_posture.py.
...
ARMS = ["A_v2_baseline", "B_v3_2_collection_fix"]
...
def format_comparison_table(a: dict, b: dict, decision: dict) -> str:
    lines = ["| Métrica | A_v2_baseline | B_v3_2_collection_fix | Δ (B-A) | Regra | Veredito |"]
    lines.append("|---------|----------------|----------------|----------|-------|----------|")
...
def rewrite_report_md(report_path: Path, metrics: dict, decision: dict, flips_text: str) -> None:
    ...
    b = metrics["B_v3_2_collection_fix"]
...
def main() -> int:
    ...
    golden_b = check_golden_cases(raw_data["B_v3_2_collection_fix"])
    decision = evaluate_pass(metrics["A_v2_baseline"], metrics["B_v3_2_collection_fix"], golden_b)
    flips = diff_per_window(raw_data["A_v2_baseline"], raw_data["B_v3_2_collection_fix"])
    ...
    print("\n=== Resumo ===")
    print(format_comparison_table(metrics["A_v2_baseline"], metrics["B_v3_2_collection_fix"], decision))
    print()
