#!/usr/bin/env python3
"""Camp 49 — Fase 2: VLMs open-weight do Bedrock vs controle Gemini-2.5, cam_picam001.
...
REVIEW_INDEX = CAM / "_review_camp49" / "INDEX.csv"
RESULTS = HERE / "results"
RESULTS.mkdir(exist_ok=True)
CSV_PATH = RESULTS / "bench_v4.csv"
SUMMARY_PATH = RESULTS / "bench_v4_summary.json"
...
# `gate_img`  = resolucao das imagens do GATE (orig|low). Em `orig` o teto de payload
#               limita o gate a 9 frames; em `low` cabem os 48.
# `gate_mids` = numero de frames intermediarios (o gate manda 1o + mids + ultimo).
#               O Camp 48 usava 3 fixo -> 5 frames. Medido: imagem original custa
#               ~1400 tok e a 640px ~324, entao 15 frames em low sai MAIS BARATO que
#               os 5 originais de antes.
PROFILES = {
    # herdados do Camp 48, para ancorar a comparacao
    "casc_v1_low":      {"stages": 2, "prompt": "v1", "img": "low", "gate_img": "orig", "gate_mids": 3},
    "casc_g3_low":      {"stages": 2, "prompt": "g3", "img": "low", "gate_img": "orig", "gate_mids": 3},
    # V4: flagrante + clausula de catador
    "v4_casc":          {"stages": 2, "prompt": "v4", "img": "low", "gate_img": "low", "gate_mids": 13},
    "v4_casc_5f":       {"stages": 2, "prompt": "v4", "img": "low", "gate_img": "low", "gate_mids": 3},
    "v4_single":        {"stages": 1, "prompt": "v4", "img": "low"},
    "v4_single_mosaic": {"stages": 1, "prompt": "v4", "img": "mosaic"},
}
GEMINI_ARMS = {
    # `current` (V1) NAO e re-rodado: as decisoes por evento vem do Camp 48 e sao
    # reavaliadas contra os rotulos novos por scripts/import_control.py (offline).
    # `current_v4` roda de verdade: sem ele a campanha compararia open-weight+V4
    # contra Gemini+V1, dando ao candidato uma alavanca negada ao controle.
    "current_v4": {"provider": "gemini", "gate": "gemini-2.5-flash-lite",
                   "detail": "gemini-2.5-flash", "prompt": "v4"},
}
...
def resolve_arm(name: str) -> dict:
    ...
    # cascata HETEROGENEA: "<gate>+<detail>:<perfil>" (ex.: kimi-k2.5+qwen3-vl-235b:v4_casc).
    # O Camp 48 forcava gate=detail=alias, e nunca reproduziu a arquitetura de prod
    # (gate barato + detail forte). A simulacao offline mostrou que `qwen` como detail
    # e o melhor combo disponivel.
    if "+" in alias:
        g_alias, d_alias = alias.split("+", 1)
    else:
        g_alias = d_alias = alias
    for a in (g_alias, d_alias):
        if a not in bc.MODELS:
            raise SystemExit(f"alias '{a}' desconhecido")
        if a in bc.ELIMINATED:
            raise SystemExit(f"alias '{a}' foi ELIMINADO pelo probe: {bc.MODELS[a].note}")
    if prof not in PROFILES:
        raise SystemExit(f"perfil '{prof}' desconhecido — {sorted(PROFILES)}")
    return dict(PROFILES[prof], provider="bedrock", alias=d_alias, name=name,
                gate_alias=g_alias, detail_alias=d_alias, gate=g_alias, detail=d_alias)


# ── Fase 0: portão de validação humana ───────────────────────────────────────
def load_review(strict=True) -> set[str]:
    """Portão da Fase 0. No Camp 49 a revisão não EXCLUI eventos — ela CORRIGE rótulos,
    e a correção já foi aplicada no disco por `apply_relabel.py`. Aqui só se confere
    que a revisão terminou e que o disco reflete o veredicto, para não medir contra um
    dataset a meio caminho. `indefinido` já sai das métricas por categoria."""
    if not REVIEW_INDEX.exists():
        raise SystemExit(f"Fase 0 não rodada: {REVIEW_INDEX} não existe.\n"
                         f"rode: python scripts/export_review.py --only <eventos>")
    raw = REVIEW_INDEX.read_text(encoding="utf-8-sig")
    head = raw.splitlines()[0]
    delim = ";" if head.count(";") > head.count(",") else ","   # Excel pt-BR salva com ;
    rows = list(csv.DictReader(raw.splitlines(), delimiter=delim))
    pend = [r["event_id"] for r in rows if not (r.get("VEREDITO") or "").strip()]
    if pend and strict:
        raise SystemExit(f"{len(pend)} de {len(rows)} eventos sem VEREDITO em "
                         f"{REVIEW_INDEX.name}: {', '.join(pend[:8])}")
    incoerente = [r["event_id"] for r in rows
                  if (r.get("VEREDITO") or "").strip().lower() == "descarte"
                  and not (CAM / "tp" / r["event_id"]).is_dir()]
    if incoerente and strict:
        raise SystemExit(
            f"veredicto 'descarte' não aplicado no disco: {', '.join(incoerente)}\n"
            f"rode: python scripts/apply_relabel.py --apply")
    return set()
...
def prompts_for(kind: str, version: str) -> str:
    if kind == "gate":
        return {"v4": p_v4.V4_GATE_PROMPT, "g3": p_g3.G3_GATE_PROMPT}.get(
            version, dg.NEW_LITTER_SYSTEM_PROMPT)
    return {"v4": p_v4.V4_DETAIL_PROMPT, "g3": p_g3.G3_DETAIL_PROMPT}.get(
        version, dg.SYSTEM_PROMPT)


COLS = ["arm", "provider", "model_id", "event_id", "det_id", "category",
        "n_raw", "n_window", "n_images_sent", "img_mode", "n_dropped_payload",
        "payload_mb", "gate_img", "gate_n_images", "gate_model", "detail_model", "prompt", "json_mode",
...
def blank_rec(ev, arm, spec, win):
    return {"arm": arm, "provider": spec["provider"],
            "model_id": (bc.MODELS[spec.get("detail_alias", spec["alias"])].model_id
                     if spec["provider"] == "bedrock" else ""),
            ...
            "gate_img": "", "gate_n_images": "", "gate_model": spec["gate"],
...
def run_bedrock(ev, spec, win, rec):
    g_alias = spec.get("gate_alias", spec["alias"])
    d_alias = spec.get("detail_alias", spec["alias"])
    alias = d_alias
    ...
    try:
        ...
        if spec["stages"] == 2:
            mids = gate_mids(win, spec.get("gate_mids")) or []
            gframes = [win[0]] + mids + [win[-1]]
            gpay = bc.prepare_images(gframes, mode=spec.get("gate_img", "orig"))
            rec["gate_img"] = spec.get("gate_img", "orig")
            ...
            g = bc.converse(g_alias, prompts_for("gate", pv), guser, gpay.blobs,
            ...
            tin, tout = g.tok_in, g.tok_out
            cost_total = bc.cost(g_alias, g.tok_in, g.tok_out)   # gate pode ser outro modelo
            if not g.json_valid:
        ...
        d = bc.converse(d_alias, prompts_for("detail", pv), duser, dpay.blobs,
...
def run_one(ev, arm_name, spec, dry=False):
    ...
    if dry:
        ...
        gf = [win[0]] + (gate_mids(win, spec.get("gate_mids")) or []) + [win[-1]]
        if spec["provider"] == "bedrock" and spec.get("stages") == 2:
            gp = bc.prepare_images(gf, mode=spec.get("gate_img", "orig"))
            rec["gate_n_images"] = gp.n_images
            rec["gate_img"] = f'{spec.get("gate_img","orig")}:{gp.raw_bytes/1e6:.2f}MB'
        rec["gate_conf"] = len(gf)
        return rec
    if spec["provider"] == "bedrock":
...
def main():
    ...
    if a.tag:
        CSV_PATH = RESULTS / f"bench_v4_{a.tag}.csv"
        SUMMARY_PATH = RESULTS / f"bench_v4_{a.tag}_summary.json"
    if a.aggregate_only:
        aggregate()
        return
