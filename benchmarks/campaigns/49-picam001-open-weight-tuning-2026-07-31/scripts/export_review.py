#!/usr/bin/env python3
"""Camp 49 — Fase 0: revisão humana dos eventos AMBÍGUOS do Camp 48.
...
# VEREDITO substitui o VALID do Camp 48: aqui a pergunta nao e "o evento serve?",
# e "o que este evento E de fato?" — descarte | catador | sem_ocorrencia | indefinido.
INDEX_COLS = ["VEREDITO", "NOTE", "event_id", "category", "classificacao", "det_id",
...
def main():
    ...
    ap.add_argument("--only", help="lista de event_id separados por virgula")
    ap.add_argument("--verify", action="store_true", help="só conferir, não escrever")
    args = ap.parse_args()

    evs = load_events(args.limit)
    if args.only:
        alvo = {e.strip() for e in args.only.split(",") if e.strip()}
        evs = [e for e in evs if e["event_id"] in alvo]
        faltando = alvo - {e["event_id"] for e in evs}
        if faltando:
            print("NAO ENCONTRADOS:", ", ".join(sorted(faltando)))
    ...
    print("Preencha VEREDITO com: descarte | catador | sem_ocorrencia | indefinido")
    return 0
