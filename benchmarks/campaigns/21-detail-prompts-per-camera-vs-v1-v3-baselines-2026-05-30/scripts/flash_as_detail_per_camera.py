#!/usr/bin/env python3
"""Gemini 2.5 Flash as Agent-2 with per-camera prompts (campanha 21).

Tests "trocar prompt resolve sem trocar modelo" — Flash V1 atual sempre
confirma (acc 57,7% trivial). Flash com prompts per-camera é o teste real.

Pricing Gemini 2.5 Flash: in $0.30/M, out $2.50/M.
"""
...
# Make _bench_common importable when run from /tmp
...
CACHE_PATH = Path("/tmp/flash_per_camera_cache.json")

BENCH_KEY = os.environ["GEMINI_TEST_API_KEY"]
CLIENT = genai.Client(api_key=BENCH_KEY)


def call_flash(paths, frame_names, cam_id, prompt_version: str):
...
def main():
    ...
    for i, ev in enumerate(events, 1):
        ...
        try:
            obj, in_tok, out_tok, latency = call_flash(paths, names, ev["cam"], prompt_version)
    ...
    print_summary("Gemini Flash 2.5", results, total_in, total_out, cost)

    out = Path("/tmp/flash_per_camera_results.json")
