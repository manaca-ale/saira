...
- B_v3_1_stricter: prompt_version="v3" (V3.1 prompt mais conservador + 3-signal gate)
...
def run_gate_call(window: Window, prompt_version: str) -> dict:
    ...
    try:
        ...
        request_id = f"bench13-{window.window_id[:12]}-{prompt_version}-{uuid.uuid4().hex[:4]}"
        t0 = time.monotonic()
...
ARM_SPECS = {
    ...
    "B_v3_1_stricter":   {"prompt_version": "v3", "description": "V3.1 (posture stricter + 3-signal gate + neg examples)"},
}
...
def main() -> int:
    ...
    p.add_argument("--arms", default="A_v2_baseline,B_v3_1_stricter")
