...
Reads results-{A_v2_baseline,B_v3_1_stricter}.json from bench_prompt_v3_posture.py.
...
ARMS = ["A_v2_baseline", "B_v3_1_stricter"]
...
def format_comparison_table(a: dict, b: dict, decision: dict) -> str:
    lines = ["| Métrica | A_v2_baseline | B_v3_1_stricter | Δ (B-A) | Regra | Veredito |"]
    lines.append("|---------|----------------|----------------|----------|-------|----------|")
...
def main() -> int:
    ...
    golden_b = check_golden_cases(raw_data["B_v3_1_stricter"])
    decision = evaluate_pass(metrics["A_v2_baseline"], metrics["B_v3_1_stricter"], golden_b)
    flips = diff_per_window(raw_data["A_v2_baseline"], raw_data["B_v3_1_stricter"])
    ...
    print("\n=== Resumo ===")
    print(format_comparison_table(metrics["A_v2_baseline"], metrics["B_v3_1_stricter"], decision))
    print()
